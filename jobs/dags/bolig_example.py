...
print(
    f"[+] Start {workers} threads for {page_size} pagesize per call: start at page {start_page} and at page {end_page} \n"
)
homes.get_pages(
...
_ = {
    boliga_recent.get_page(page=page, pagesize=15, verbose=True)
...
print(
    f"Data storesd {pd.concat(nybolig.store.values(), ignore_index=True).shape[0]} rows from nybolig.dk\n"
)
...
print(
    f"[+] Estate | Start {workers} threads for {page_size} pagesize per call: start at page {start_page} and at page {end_page} \n"
)
estate.get_pages(
    start_page=start_page,
    end_page=end_page,
...
print(
    f"[+] Nybolig | Start {workers} threads for {page_size} pagesize per call: start at page {start_page} and at page {end_page} \n"
)
nybolig.get_pages(
    start_page=start_page,
    end_page=end_page,
