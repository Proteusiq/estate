...
| `estates/`       | A Python directory that contains code for your Dagster repository                 |
| `estates_tests/` | A Python directory that contains tests for `estates`                      |
...
pyenv virtualenv $PYTHON_VERSION estates
pyenv activate estates
```

**Conda**
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name estates python=PYTHON_VERSION
conda activate estates
```
...
Tests can be found in `estates_tests` and are run with the following command:

```bash
pytest estates_tests
```

As you create Dagster ops and graphs, add tests in `estates_tests/` to check that your
